...
setwd("~/ERB/EXP_046/MAT_bedgraphs")
options(stringsAsFactors=FALSE)
bedgraphDir <- "~/ERB/EXP_046/MAT_bedgraphs/" #Tells where to find the bedgraph files. 
samples <- sub("_MAT.bedgraph", "", list.files(bedgraphDir, pattern="*_MAT.bedgraph$"))
...
bedgraphList <- lapply(samples, function(s) {
  fread(paste(bedgraphDir, s, "_MAT.bedgraph" , sep=""), header=FALSE, skip=1, col.names = c("chr", "start", "end", "cov"))
})
...
fwrite(normalized_bd, "merged_normalized_MATbedgraph.txt")


normalized_bd <- fread("merged_normalized_MATbedgraph.txt")
...
df_ip237$time <- c(rep("00",11000),rep("05",11000),rep("10",11000),rep("15",11000),rep("20",11000),rep("30",11000),rep("45",11000),rep("90",11000))
df_ip238$time <-  c(rep("00",11000),rep("05",11000),rep("10",11000),rep("15",11000),rep("20",11000),rep("30",11000),rep("45",11000),rep("90",11000))
...
df_ip261$time <-  c(rep("00",11000),rep("05",11000),rep("10",11000),rep("15",11000),rep("20",11000),rep("30",11000),rep("45",11000),rep("90",11000))
df_ip262$time <-  c(rep("00",11000),rep("05",11000),rep("10",11000),rep("15",11000),rep("20",11000),rep("30",11000),rep("45",11000),rep("90",11000))
...
wrap_avg_roi <- function(chromosome, region, dat){
  ...
  ggplot(new_dat, aes(x= start, y= avg_cov))+
    geom_line(size = 1.5, color="#99CCCC")+
    theme_classic()+
...
wrap_avg_roi(chromosome="III_cer", region=c(199500,201000), dat=df_sir_wt_IP)
